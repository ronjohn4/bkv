...
from app.models import Instance, InstanceAudit ,load_user
from app.inst.forms import InstanceForm
...
def list():
    ...
    lastpagelist = page
    datalist = Instance.query.paginate(page, current_app.config['ROWS_PER_PAGE_FULL'], False)
...
def add():
    ...
    if request.method == 'POST' and form.validate_on_submit():
        var = Instance(name=request.form['name'], is_active='is_active' in request.form)
        db.session.add(var)
...
@login_required
def delete(id):
    InstanceAudit.query.filter_by(parent_id=id).delete()
...
def auditlist(id):
    ...
    audit_list = InstanceAudit.query.\
...
def addtest():
    ...
    for addone in range(add_count):
        var = Instance(name=f'name{addone}',
                  desc=f'desc{addone}',
                  is_active=0
                  )
        db.session.add(var)
...
def writeaudit(parent_id, before, after):
    if after is None:
        ...
        change = "change"
    var = InstanceAudit(parent_id=parent_id,
